...
class ZyPlayerPlugin {
	request: any = null;
	parseXml: any = null;

	constructor(parentParams: any) {
		this.request = parentParams.request;
		this.parseXml = parentParams.parseXml;
	}

	#parseXmlToData(xml: string) {
		const parseVideoList = (str: string) => {
			str = str.endsWith("#") ? str.slice(0, str.length - 1) : str;
			const arr = str.split("#");
		...
		let parseRes = this.parseXml(xml);
	...
	/** 获取当前源下面所有分类名称及ID */
	async getClassifyList(params: any) {
		const { url } = params;
		const res: any = await this.request.call(this, { url });
		if (!res) {
			return [];
		}
		const { ground } = this.#parseXmlToData.call(this, res);
		return ground;
	}
}
